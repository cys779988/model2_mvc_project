<%@page import="vo.MemberVO"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"
    isELIgnored="false" %>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>    
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<!DOCTYPE html>
<html>
<head>
<c:set var="contextPath"  value="${pageContext.request.contextPath}"  />
<meta charset="UTF-8">
<script src="http://dmaps.daum.net/map_js_init/postcode.v2.js"></script>
<script src="//code.jquery.com/jquery.min.js"></script>
<style type="text/css">
table{
	width: 1000px;
	border: 1px solid #444444;
    border-collapse: collapse;
  }
tr, td{
    border: 1px solid #444444;
  }
.btn{
display:block;
width:100%;
margin:0 auto;
text-align:center;
margin-top: 100px;
}
</style>

</head>
<c:choose>
<c:when test="${memberSession.admin eq 'AUTH_ADMIN'}">
<body>
	<form name="regist" action="regist.do" method="post" onSubmit="return checkIt()" autocomplete="off">
		<h1 class="text_center">직원 등록</h1>
		<table align="center">
			<tr>
				<td width="100"><p align="right">성명*</td>
				<td width="400"><input type="text" name="name" maxlength="6"></td>
				<td width="100"><p align="right">생년원일</td>
				<td width="400">
				<input type="date" name="birth"></td>
			</tr>
			<tr>
				<td width="100"><p align="right">연락처</td>
				<td width="100"><p>
						<input type="text" name="phone" id="phone" maxlength="13"></td>
				<td width="100"><p align="right">성별</td>
				<td width="400"><input type="radio" name="gen" value="GENDER_MALE" checked="checked" /> 남자
				<input type="radio" name="gen" value="GENDER_FEMALE" /> 여자</td>
			</tr>
			<tr>
				<td width="100"><p align="right">사번*</td>
				<td width="400"><input type="text" name="empNo" id="empNo" maxlength="6">
				<input type="button" name="confirm_id" value="사원번호 입력받기" onClick="openConfirmid(this.form)">
				<td width="100"><p align="right">입사일자*</td>
				<td width="400"><input type="date" name="joinDate"></td>
			</tr>
			<tr>
				<td width="100"><p align="right">비밀번호</td>
				<td width="400"><input type="password" name="pwd" maxlength="10"><br></td>
				<td width="100"><p align="right">내선번호</td>
				<td width="50"><input type="text" name="tel" id="tel" maxlength="12"></td>
			</tr>
			<tr>
				<td width="100"><p align="right">권한*</td>
				<td width="400"><p>
						<input type="checkbox" name="admin" value="AUTH_ADMIN"></td>
				<td width="100"><p align="right">직급*</td>
				<td width="400"><select name="position">
						<option value="P10">대표이사</option>
						<option value="P09">부사장</option>
						<option value="P08">전무</option>
						<option value="P07">본부장</option>
						<option value="P06">부장</option>
						<option value="P05">차장</option>
						<option value="P04">과장</option>
						<option value="P03">대리</option>
						<option value="P02">주임</option>
						<option value="P01">사원</option>
				</select></td>
			</tr>
			<tr>
				<td width="100"><p align="right">이메일</td>
				<td width="400"><input type="text" name="email" maxlength="10">
					@forwiz.com</td>
				<td width="100"><p align="right">부서*</td>
				<td width="400"><select name="dept">
							<option value="D10">대표이사</option>
							<option value="D09">경영지원본부</option>
							<option value="D08">전략사업본부</option>
							<option value="D07">SI사업본부</option>
							<option value="D06">SI사업4팀</option>
							<option value="D05">SI사업3팀</option>
							<option value="D04">SI사업2팀</option>
							<option value="D03">SI사업1팀</option>
							<option value="D02">부설연구소</option>
							<option value="D01">경영지원팀</option>
						</select>
			</tr>
			<tr>
				<td width="100"><p align="right" />주소</td>
				<td colspan="3"><input type="text" id="addr" name="addr" maxlength="50" style="width: 500px;">
				<!-- <td colspan="3">
				<input type="text" id="zip" name="zip" style="width: 100px;" placeholder="우편번호">
				<input type="button" onclick="execDaumPostcode()" value="우편번호 찾기">
				<input type="text" id="addr1" name="addr1" style="width: 200px;" placeholder="주소">
				<input type="text" id="addr2" name="addr2" style="width: 200px;" placeholder="상세주소"></td> -->
				</tr>
		</table>
				<div class="btn">
				<input type="submit" value="가입하기">
				<input type="reset" value="다시입력">
				<input type="button" value="목록으로 이동" onclick="location.href='/forwiz2/list.do'"></div>
	</form>
<script>
	function checkIt() {
		var regist = document.regist;
/* 		if (!regist.empNo.value) {
			alert("사원번호를 입력하세요");
			regist.empNo.focus();
			return false;
		} */
		if (!regist.name.value) {
			alert("사용자 이름을 입력하세요");
			regist.name.focus();
			return false;
		}
		if (!regist.joinDate.value) {
			alert("입사일자를 입력하세요");
			return false;
		}
	 	if(document.getElementById("phone").value.length>13) {
			alert("연락처를 다시 입력해주세요")
			document.getElementById("phone").value="";
			regist.phone.focus();
			return false;
		}
	 	if(document.getElementById("tel").value.length>12) {
			alert("내선번호를 다시 입력해주세요")
			document.getElementById("tel").value="";
			regist.tel.focus();
			return false;
		}
		if (!regist.dept.value) {
			alert("부서명을 입력하세요");
			regist.dept.focus();
			return false;
		}
		if (!regist.position.value) {
			alert("직급을 입력하세요");
			regist.position.focus();
			return false;
		}
		return true;
	}
	
	//연락처 (-)붙이기
	/* $(document).on("keyup", "#phone", function() { $(this).val( $(this).val().replace(/[^0-9]/g, "").replace(/(^02|^0505|^1[0-9]{3}|^0[0-9]{2})([0-9]+)?([0-9]{4})$/,"$1-$2-$3").replace("--", "-") ); }); */
	$(document).on("keyup", "#phone", function() { $(this).val( $(this).val().replace(/[^0-9]/g, "").replace(/(^02|^[0-9][0-9]{2})([0-9]+)?([0-9]{4})$/,"$1-$2-$3").replace("--", "-") ); }); 
	$(document).on("keyup", "#tel", function() { $(this).val( $(this).val().replace(/[^0-9]/g, "").replace(/(^02|^[0-9][0-9]{2})([0-9]+)?([0-9]{4})$/,"$1-$2-$3").replace("--", "-") ); }); 
	/* $(document).on("keyup", "#phone", function() { 
		var regtel = /^[0-9]*$/;
		phone = $(this).val();
		if(!regtel.test(phone)){
			alert("길이가 너무깁니다.");
			$(this).val($(this).val().replace(/[^0-9]/g,""));
		}
	}); */
	/* $(document).on("keyup", "#tel", function() { 
		var regtel = /^[0-9]*$/;
		tel = $(this).val();
		if(!regtel.test(tel)){
			alert("숫자만 입력하세요");
			$(this).val($(this).val().replace(/[^0-9]/g,""));
		}
		
	}); */
	
	//중복되지 않는 사원번호를 자동으로 가져옴
	function openConfirmid(regist){
		var Now = new Date();
		var yy = String(Now.getFullYear());
		var m = Now.getMonth()+1;
		var n = 1;
		if(m<10){m = "0" + m};

		var y = yy.substr(2,2);
		var empNo = y+m+"0"+n;
		url = "confirm.do?empNo="+empNo;
		open(url, "confirm", "toolbar=no,location=no,status=no,menubar=no,scrllbar=no,resizable=no,width=300,height=200");
}

	//다음지도 주소검색
	/* function execDaumPostcode() {
		new daum.Postcode({
			oncomplete : function(data) {
				var addr = ''; // 주소 변수
				var extraAddr = ''; // 참고항목 변수
				if (data.userSelectedType === 'R') {
					addr = data.roadAddress;
				} else {
					addr = data.jibunAddress;
				}
				if (data.userSelectedType === 'R') {
					if (data.bname !== '' && /[동|로|가]$/g.test(data.bname)) {
						extraAddr += data.bname;
					}
					if (data.buildingName !== '' && data.apartment === 'Y') {
						extraAddr += (extraAddr !== '' ? ', '
								+ data.buildingName : data.buildingName);
					}
				}
				document.getElementById('zip').value = data.zonecode;
				document.getElementById("addr1").value = addr;
				document.getElementById("addr2").focus();
			}
		}).open();
	} */

</script>
</c:when>
<c:otherwise>
<%
out.println("<script>window.onload=alert('권한이 없습니다.')</script>");
out.println("<script>location.href=\"list.do\";</script>");
%>
</c:otherwise>
</c:choose>
</body>
</html>